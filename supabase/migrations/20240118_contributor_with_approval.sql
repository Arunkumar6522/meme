-- Add Contributor Feature with ADMIN APPROVAL
-- Users request → PENDING → Admin approves → ACTIVE
...
CREATE POLICY "Users can update own profile"
    ...
    WITH CHECK (
        id = auth.uid() AND
        -- Users can only update their own contributor fields, not role or approval status
...
CREATE OR REPLACE FUNCTION public.request_contributor(
    p_name TEXT,
    p_email TEXT,
    p_mobile TEXT
)
...
BEGIN
    ...
    -- Update user to PENDING contributor
    UPDATE public.users
    SET 
...
CREATE OR REPLACE FUNCTION public.approve_contributor(
    p_user_id UUID,
...
CREATE OR REPLACE FUNCTION public.get_pending_contributors()
RETURNS TABLE (
    user_id UUID,
    email TEXT,
